...
public abstract class TableHandler<T extends Base> {
	...
	protected TableHandler(ConsumerConfig cfg) {
		config = cfg; queue = new ArrayBlockingQueue<>(config.getQueueCapacity());
		MAX_BATCH_SIZE = config.getBatchSize(); FLUSH_INTERVAL_MS = config.getFlushIntervalMs();
	}
	...
	final void handle(Struct payload, Class<T> entityClass) {
		...
		case OPERATION.d -> delete(entityMapper.mapStructToEntity(payload.getStruct(BEFORE), entityClass)); default -> {
	...
	final void handleQueue(Struct payload, Class<T> entityClass) {
		try {
			OPERATION operation = OPERATION.valueOf(payload.getString(OP)); switch (operation) {
			case OPERATION.c, OPERATION.u, OPERATION.r ->
	...
	final void processQueue(Class<T> entityClass) {
		final List<T> wlist = new ArrayList<>(); final List<T> dlist = new ArrayList<>();
		OPERATION previous = OPERATION.d; long lastFlush = System.currentTimeMillis(); try {
			do {
				T entity = queue.take();
				if (entity.operation() == OPERATION.c || entity.operation() == OPERATION.u || entity.operation() == OPERATION.r) {
					if (previous == OPERATION.d) {
						delete(dlist); dlist.clear(); previous = OPERATION.c;
					} wlist.add(entity);
					if (wlist.size() >= MAX_BATCH_SIZE || (System.currentTimeMillis() - lastFlush) >= FLUSH_INTERVAL_MS || queue.isEmpty()) {
						save(wlist); wlist.clear(); lastFlush = System.currentTimeMillis();
					}
				}
				else {
					if (previous == OPERATION.c) {
						save(wlist); wlist.clear(); previous = OPERATION.d;
					} dlist.add(entity);
					if (dlist.size() >= MAX_BATCH_SIZE || (System.currentTimeMillis() - lastFlush) >= FLUSH_INTERVAL_MS || queue.isEmpty()) {
						delete(dlist); dlist.clear(); lastFlush = System.currentTimeMillis();
					}
				}
			}
			while (true);
			...
			log.warn("Interrupted while waiting for queue to be empty, persisting the pending in-flights. " + "Queue Size: " + queue.size() + " In-flight create/update list size: " + wlist.size() + " In-flight delete list size: " + dlist.size());
			if (previous == OPERATION.c) {
				save(wlist); delete(dlist);
			}
			else {
				delete(dlist); save(wlist);
			}
		}
	}

	protected void offer(OPERATION key, T value) throws InterruptedException {
		value.operation(key); queue.put(value);
	}

	@PreDestroy
	final void gracefulDrain() {
		var consumer = this.getClass().getSimpleName(); try {
			log.info("Initiated graceful drain"); if (!queue.isEmpty()) {
				log.info("Pending events to be processed in : " + consumer + ": " + queue.size());
				Thread.sleep(config.getDrainIntervalMs()); if (!queue.isEmpty()) {
					log.error("Unable to gracefully process events in " + consumer + ": " + queue.size() + " events left to be processed");
				}
				else {
